...
module.exports = function (THREE) {
	...
	THREE.GLTF2Loader = (function () {
		...
		var EXTENSIONS = {
			...
			KHR_LIGHTS: 'KHR_lights_cmn',
			KHR_LIGHTS_PBR: 'KHR_lights_pbr',
			KHR_MATERIALS_COMMON: 'KHR_materials_common',
			// not in use ATM, but defined in gltf by blender:
			KHR_MATERIALS_cmnBlinnPhong: 'KHR_materials_cmnBlinnPhong',
			// not in use ATM, but defined in gltf by blender:
		...
		function GLTFLightsExtension(json) {
			...
			var extensionPbr = (json.extensions && json.extensions[EXTENSIONS.KHR_LIGHTS_PBR]) || {};
			var lights = extension.lights || {};
			var lightsPbr = extensionPbr.lights || {};
			...
			for (var lightId in lights) {
				...
				switch (light.type) {
					...
					case 'directional':
						lightsPbr.forEach(lightPbr => {
							if (lightPbr.name === light.name) {
								color = new THREE.Color().fromArray(lightPbr.color);
								lightNode = new THREE.DirectionalLight(color);
								lightNode.intensity = lightPbr.strength / 5;
								lightNode.castShadow = true;
					...
					case 'point':
						lightsPbr.forEach(lightPbr => {
							if (lightPbr.name === light.name) {
								...
								lightNode.intensity = lightPbr.strength / 30;
								// lightNode.intensity = 0;
								lightNode.castShadow = true;
							}
						});
					...
					case 'spot':
						lightsPbr.forEach(lightPbr => {
							if (lightPbr.name === light.name) {
								...
								lightNode.decay = 1;
								lightNode.power = lightPbr.strength / 5;
								// lightNode.power = 0;
								lightNode.angle = light.positional.spot.outerAngle;
								// lightNode.angle = Math.PI / 2;
								// lightNode.angle = 1.0472;
								lightNode.penumbra = 0.3;
								lightNode.distance = 20;
								lightNode.castShadow = true;
							}
						});
						// lightNode.position.set(0, 1, 0);
						break;

					case 'ambient':
						lightNode = new THREE.AmbientLight(new THREE.Color("rgb(100, 100, 100)"));
						break;
		...
		GLTFParser.prototype.loadNodes = function () {
			...
			var extensions = this.extensions;
			var scope = this;
			scope.camTargetNodes = [];

			var nodes = json.nodes || [];
			var skins = json.skins || [];

			var path = './static_assets/Park2/';
			var format = '.jpg';
			var urls = [
			...
			return _each(json.nodes, function (node) {
				...
				return scope._withDependencies([
					...
					return _each(__nodes, function (_node, nodeId) {
						...
						_node.traverse(subnode => {

							if (subnode.material) {
								if (subnode.material.isMeshStandardMaterial ||
									(subnode.material.isShaderMaterial && subnode.material.envMap !== undefined)) {

									envMap.format = THREE.RGBFormat;
									scope.envMap = envMap;
									subnode.material.envMap = envMap;
						...
						if (node.children
							&& node.children[0]
							&& json.nodes[node.children[0]]
							&& json.nodes[node.children[0]].extensions
							&& json.nodes[node.children[0]].extensions[EXTENSIONS.KHR_LIGHTS]
							&& json.nodes[node.children[0]].extensions[EXTENSIONS.KHR_LIGHTS].light !== undefined) {

							// global (?) lights from parsed at approx. line 350
							var lights = extensions[EXTENSIONS.KHR_LIGHTS].lights;
							// get light by index from this node (defined in gltf)
							var lightIndex = json.nodes[node.children[0]].extensions[EXTENSIONS.KHR_LIGHTS].light;
							var light = lights[lightIndex];
							if (light.type === 'SpotLight') {
								var targetNode = new THREE.Object3D();
								_node.add(targetNode);
								targetNode.position.copy(light.position);
								targetNode.position.y = targetNode.position.y - 10;
								light.target = targetNode;
								// var quaternionCorrectionArr = json.nodes[node.children[0]].rotation;
								// var quaternionCorrection = new THREE.Quaternion().fromArray(quaternionCorrectionArr);
								// light.matrixAutoUpdate = true;
								// light.applyQuaternion(quaternionCorrection);
								...
								// _node.add(camHelper);
								// var camTargetNode = new THREE.Object3D();
								// scope.camTargetNodes.push(camTargetNode);
								// light.shadow.target = camTargetNode;
								// light.target = camTargetNode;
							...
							if (['SpotLight', 'DirectionalLight', 'PointLight'].includes(light.type)) {
								// helper.matrixAutoUpdate = true;
								// _node.add(helper);
								_node.add(light);
								// scope.testLights.push(light);
							}
							console.log(light);
						}
		...
		GLTFParser.prototype.loadScenes = function () {
			...
			return this._withDependencies([
				...
				return _each(json.scenes, function (scene) {
					...
					scope.camTargetNodes.forEach(targetNode => {
						_scene.add(targetNode);
					});
					...
					scope.testLights.forEach(light => {
						// _scene.add(light);
					});

					_scene.background = scope.envMap;
